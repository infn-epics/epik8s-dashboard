...
export async function generateTaskZip(taskDef, prefix, basePath) {
  ...
  folder.file('start.sh', generateStartSh(taskDef.module));
...
export async function deployPlugin(controllerApiUrl, { name, git_url, path, pat, branch, auto_start = true }) {
  ...
  if (!resp.ok) {
    const err = new Error(data?.detail || data?.message || `Deploy failed (${resp.status})`);
    err.validation = data?.validation || null;
    throw err;
  }
  if (data?.ok === false) {
    const err = new Error(data?.message || 'Deploy was rejected by the controller');
    err.validation = data?.validation || null;
    throw err;
  }
  return data;
...
/* ────────────────────────────────────────────
   Soft IOC / ArgoCD integration helpers
   ──────────────────────────────────────────── */

export const DEFAULT_SOFTIOC_CHARTURL =
  'https://github.com/infn-epics/ioc-chart.git';
export const DEFAULT_SOFTIOC_IMAGE =
  'ghcr.io/infn-epics/epik8s-beamline-controller';
export const SOFTIOC_VALUES_PATH = 'deploy/values-softiocs.yaml';

/**
 * Generate a `start.sh` launcher script for standalone iocmng deployment.
 * This script is placed in the git config directory and is executed by the
 * ioc-chart container (via `start: /epics/ioc/config/start.sh`).
 *
 * The ioc-chart init container clones the git repo into /epics/ioc/config/,
 * so the start.sh runs from that working directory.
 */
export function generateStartSh(moduleName) {
  return `#!/bin/bash
# start.sh — Launch iocmng task "${moduleName}" as a standalone soft IOC
# Auto-generated by EPIK8s Dashboard Task Wizard.
# Runs inside the ioc-chart container after git clone into /epics/ioc/config/
set -e

TASK_DIR="/epics/ioc/config"
TASK_NAME="\${IOCMNG_TASK_NAME:-${moduleName}}"
...
# Generate plugins config pointing to the config directory
cat > /tmp/iocmng-plugins.yaml << EOF
plugins:
  - name: \${TASK_NAME}
    path: \${TASK_DIR}
    auto_start: true
...
export IOCMNG_PLUGINS_CONFIG=/tmp/iocmng-plugins.yaml
export IOCMNG_PREFIX="\${IOCMNG_PREFIX:-BEAMLINE:CONTROL}"
...
/**
 * Generate a values-softiocs.yaml entry for a new soft IOC application.
 * Produces an entry compatible with the ioc-chart Helm chart (same chart
 * used for all IOCs). The entry is placed under `softiocs.<name>` and
 * rendered by the softiocs.yaml template in epik8s-chart.
 *
 * @param {Object} taskDef — wizard task definition
 * @param {string} projectPath — git path where the task code is committed
 * @param {string} [iocPrefix] — PV prefix for this IOC (e.g. "SPARC:MY_TASK")
 * @returns {Object} entry to add under `softiocs.<name>` in values-softiocs.yaml
 */
export function generateSoftIocEntry(taskDef, projectPath, iocPrefix) {
  const name = taskDef.module || taskDef.name;
  return {
    name,
    iocprefix: iocPrefix || '',
    module: name,
    ...
    enable_ingress: true,
    env: [
      { name: 'IOCMNG_PREFIX', value: iocPrefix || 'BEAMLINE:CONTROL' },
      { name: 'IOCMNG_TASK_NAME', value: name },
      { name: 'IOCMNG_LOG_LEVEL', value: 'info' },
    ],
    service: {
...
/**
 * Merge a new soft IOC entry into an existing values-softiocs.yaml string.
 * If the file doesn't exist yet (empty string), creates a fresh document.
 * @param {string} existingYaml — current YAML content (may be empty)
 * @param {Object} entry — from generateSoftIocEntry()
 * @returns {string} updated YAML content
 */
export function mergeSoftIocValues(existingYaml, entry) {
  let doc;
  try {
  ...
  if (!doc.softiocs || typeof doc.softiocs !== 'object') {
    doc.softiocs = {};
  }
  const key = entry.name || entry.module;
  doc.softiocs[key] = entry;
  return (
    `# values-softiocs.yaml — Soft IOC applications managed by EPIK8s Dashboard\n` +
    `# Each entry creates an ArgoCD Application via the softiocs chart template.\n` +
    `# Managed automatically — manual edits will be preserved on next update.\n\n` +
