...
import { formatPvValue, getPvText } from '../../../components/common/PvControls.jsx';
  ...
  return (
    <div className="timing-widget-body timing-essential">
      ...
      <div className="timing-field-row">
        <PvDisplay client={client} pvName={evgPrefix ? `${evgPrefix}:EvtClkFrequency-RB` : ''} label="Evt Clk" precision={3} format={numberFormat} showUnit={showUnits} unit="MHz" />
      </div>
          ...
          <table className="timing-dg-table">
            ...
            <tbody>
              {Array.from({ length: numDG }, (_, i) => (
                <DGRowEssential key={i} idx={i} evrPrefix={evrPrefix} client={client} precision={precision} numberFormat={numberFormat} />
              ))}
            </tbody>
          </table>
        </div>
...
function DGRowEssential({ idx, evrPrefix, client, precision, numberFormat }) {
  ...
  return (
    <tr>
      ...
      <td>{formatPvValue(delay?.value, numberFormat, precision)}</td>
      <td>{formatPvValue(width?.value, numberFormat, precision)}</td>
      <td>{evt?.value ?? '—'}</td>
    </tr>
  );
}
...
function TimingDetail({ config, client }) {
  ...
  return (
    <div className="timing-widget-body timing-detail">
      ...
      {tab === 'evg' && <EvgClockTab evgPrefix={evgPrefix} client={client} precision={precision} numberFormat={numberFormat} showUnits={showUnits} />}
      {tab === 'mxc' && <MxcTab evgPrefix={evgPrefix} client={client} numberFormat={numberFormat} showUnits={showUnits} />}
      {tab === 'trigevt' && <TrigEvtTab evgPrefix={evgPrefix} client={client} numberFormat={numberFormat} showUnits={showUnits} />}
      {tab === 'dlygen' && <DlyGenTab evrPrefix={evrPrefix} client={client} numDG={numDG} precision={precision} numberFormat={numberFormat} showUnits={showUnits} />}
      {tab === 'outputs' && <OutputsTab evrPrefix={evrPrefix} client={client} />}
      {tab === 'sfp' && <SfpTab evgPrefix={evgPrefix} evrPrefix={evrPrefix} client={client} numberFormat={numberFormat} showUnits={showUnits} />}
    </div>
  );
}
...
function EvgClockTab({ evgPrefix, client, precision, numberFormat, showUnits }) {
  return (
    <div className="timing-tab-body">
      <div className="timing-section-title">Event Clock</div>
      <div className="timing-field-row">
        <PvDisplay client={client} pvName={evgPrefix ? `${evgPrefix}:EvtClkFrequency-RB` : ''} label="Frequency" precision={precision} format={numberFormat} showUnit={showUnits} unit="MHz" />
...
function MxcTab({ evgPrefix, client, numberFormat, showUnits }) {
      ...
      <table className="timing-dg-table">
        ...
        <tbody>
          {mxcs.map(n => (
            <tr key={n}>
              <td>Mxc{n}</td>
              <td><PvDisplay client={client} pvName={evgPrefix ? `${evgPrefix}:Mxc${n}Prescaler-SP` : ''} precision={0} format={numberFormat} showUnit={showUnits} /></td>
              <td><PvDisplay client={client} pvName={evgPrefix ? `${evgPrefix}:Mxc${n}Frequency-RB` : ''} precision={3} format={numberFormat} showUnit={showUnits} /></td>
            </tr>
          ))}
        </tbody>
      </table>
...
function TrigEvtTab({ evgPrefix, client, numberFormat, showUnits }) {
      ...
      <table className="timing-dg-table">
        ...
        <tbody>
          {events.map(n => (
            <tr key={n}>
              <td>TrigEvt{n}</td>
              <td><PvDisplay client={client} pvName={evgPrefix ? `${evgPrefix}:TrigEvt${n}EvtCode-SP` : ''} precision={0} format={numberFormat} showUnit={showUnits} /></td>
...
function DlyGenTab({ evrPrefix, client, numDG, precision, numberFormat, showUnits }) {
  ...
  return (
    <div className="timing-tab-body">
      ...
      <table className="timing-dg-table timing-dg-detail">
        ...
        <tbody>
          {Array.from({ length: numDG }, (_, i) => (
            <DGRowDetail key={i} idx={i} evrPrefix={evrPrefix} client={client} precision={precision} numberFormat={numberFormat} showUnits={showUnits} />
          ))}
        </tbody>
      </table>
    </div>
...
function DGRowDetail({ idx, evrPrefix, client, precision, numberFormat, showUnits }) {
  const label = usePv(client, `${evrPrefix}:DlyGen${idx}Label-I`);
  ...
  return (
    <tr>
      ...
      <td><PvDisplay client={client} pvName={`${evrPrefix}:DlyGen${idx}EvtTrig0-SP`} precision={0} format={numberFormat} showUnit={showUnits} /></td>
    </tr>
  );
}
...
function SfpTab({ evgPrefix, evrPrefix, client, numberFormat, showUnits }) {
  return (
    <div className="timing-tab-body">
      {evgPrefix && (
        <>
      ...
      {evrPrefix && (
        <>
          ...
          <div className="timing-field-row">
            ...
            <PvDisplay client={client} pvName={`${evrPrefix}:LinkClk-I`} label="Link Clk" unit="MHz" precision={3} format={numberFormat} showUnit={showUnits} />
          </div>
          <div className="timing-field-row">
            <PvDisplay client={client} pvName={`${evrPrefix}:CntRxErr-I`} label="RX Err" precision={0} format={numberFormat} showUnit={showUnits} />
            <PvDisplay client={client} pvName={`${evrPrefix}:CntLinkTimo-I`} label="Timeouts" precision={0} format={numberFormat} showUnit={showUnits} />
            <PvDisplay client={client} pvName={`${evrPrefix}:Time-I`} label="Time" />
          </div>

          <div className="timing-section-title">EVR SFP</div>
          <div className="timing-field-row">
            <PvDisplay client={client} pvName={`${evrPrefix}:SFPPwrRX-I`} label="RX Power" unit="mW" precision={3} format={numberFormat} showUnit={showUnits} />
            <PvDisplay client={client} pvName={`${evrPrefix}:SFPPwrTX-I`} label="TX Power" unit="mW" precision={3} format={numberFormat} showUnit={showUnits} />
            <PvDisplay client={client} pvName={`${evrPrefix}:SFPT-I`} label="Temp" unit="°C" precision={1} format={numberFormat} showUnit={showUnits} />
          </div>
          <div className="timing-field-row">
